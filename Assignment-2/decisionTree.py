...
decTree.fit(input_train,result_train)

#Unbalanced model
decTreeUnbalanced = DecisionTreeClassifier()
decTreeUnbalanced.fit(input_train,result_train)

#Test model, Accuracy
result_prediction = decTree.predict(input_test)
...
#Test model, Accuracy, Unbalanced, no depth limit
print("##### Unbalanced Model!")
result_prediction = decTreeUnbalanced.predict(input_test)
accuracy = accuracy_score(result_test, result_prediction)
accuracy_count = accuracy_score(result_test, result_prediction, normalize=False)
print(f"Accuracy: {accuracy*100 :.2f}%")
print(f"Accuracy Count: {accuracy_count :.0f} out of {len(input_test)}")
